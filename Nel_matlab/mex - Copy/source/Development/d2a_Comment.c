...
*
* d2a.c
* mex file for D/A output from 6052e card, playing out sound files and changing their sampling rate. Now using PCIe-6341
*
...
//	__asm int 3h ; // Setting a breakpoint for debugging (compile with the -g flag!)
	...
	if (iMode == 1) { 
		...
		/**********************************************************************/
		// Mex: initialize piLoadBuffer and make it persistent
		if (!bPIBuffer_initialized)
		{
			mexPrintf("Mex-file <d2a> is allocating memory (%i points) for piLoadBuffer.\n", ulMAX_BUFFER_PNTS);
			piLoadBuffer = mxCalloc(ulMAX_BUFFER_PNTS, sizeof(i16));//Allocate dynamic memory for array,ulMAX_BUFFER_PNTS: Number of elements to allocate, sizeof(i16): Number of bytes per element, (The C sizeof operator calculates the number of bytes per element.)///???piLoadBuffer
			mexMakeMemoryPersistent(piLoadBuffer);//Make memory allocated by MATLAB software persist after MEX-function completes, piLoadBuffer: Pointer to the beginning of memory allocated by one of the MATLAB® memory allocation routines
			mexAtExit(cleanup);//Register function to call when MEX function clears or MATLAB terminates
			bPIBuffer_initialized = 1;
		}

		/**********************************************************************/
        // Mex: sampling rate and .wav file are fed to input arguments of mexFunction
		dRequestedUpdateRate_Hz = (f64) mxGetScalar(prhs[1]);   // input argument 
		iNumChans = (i16) mxGetN(prhs[2]);          // # of columns in samples matrix (input argument)///??? number of channels
		ulCount = (u32) mxGetM(prhs[2]);			// # of rows in samples matrix (input argument)///??? two .wav files have the same length
		pdBuffer0 = (f64*) mxGetPr(prhs[2]);		// pointer to data of first column of samples matrix, pointer to data Buffer 0
		pdBuffer1 = &(pdBuffer0[ulCount]);			// pointer to data of second column of samples matrix, pointer to data Buffer 1///???

        /******************************************************************************/
        // Mex: getting actual sampling rate according to the requested updated sampling rate
		...
		// Set Clock using computed ulUpdateInt, and iUpdateTB=-3 (set at init)///??? iUpdateTB=-3
