...
if ~out_of_range,
   ...
   crmag = CDATA(iseq,2) - SRdata.dBV;
   crphs = CDATA(iseq,3);
   if isntrp
      factor = (thfreq - CDATA(iseq,1))/(CDATA(iseq+1,1) - CDATA(iseq,1));
      crmag = crmag + factor*(CDATA(iseq+1,2) - SRdata.dBV - crmag);
      crphs = crphs + factor*(CDATA(iseq+1,3) - crphs);
   end
   COMM.SRdata.dbspl = DDATA(FREQS.ndpnts,2) + crmag;
