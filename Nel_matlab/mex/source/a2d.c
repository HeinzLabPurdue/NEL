...
//     /*********************************************/
        ...
        status[6] = DAQmxCfgSampClkTiming(taskTriggerAI,"/DAQ/PFI6",100000,DAQmx_Val_Rising,DAQmx_Val_ContSamps,10000000);
    ...
    *********************************************/
        ...
        double *prevIDX, *currIDX, *prevTriggerIDX, *currTriggerIDX;
        /*********************************************/
        ...
        /*********************************************/
        plhs[2] = mxCreateDoubleMatrix(read,1,mxREAL);
        samples = mxGetPr(plhs[2]);
        for (i=0; i<(int)read;i++){
            samples[i] = data[i];
        }
        
        plhs[3] = mxCreateDoubleMatrix(readTrigger,1,mxREAL);
        triggerSamples = mxGetPr(plhs[3]);
        for (i=0; i<(int)readTrigger;i++){
            triggerSamples[i] = dataTrigger[i];
        }
        
        plhs[4] = mxCreateDoubleMatrix(1,1,mxREAL);
        prevIDX = mxGetPr(plhs[4]);
        prevIDX[0] = totalRead;
        
        plhs[5] = mxCreateDoubleMatrix(1,1,mxREAL);
        prevTriggerIDX = mxGetPr(plhs[5]);
        prevTriggerIDX[0] = cSeq;
        ...
        currIDX = mxGetPr(plhs[6]);
        currIDX[0] = totalRead;
        
        plhs[7] = mxCreateDoubleMatrix(1,1,mxREAL);
        currTriggerIDX = mxGetPr(plhs[7]);
        currTriggerIDX[0] = cSeq;
