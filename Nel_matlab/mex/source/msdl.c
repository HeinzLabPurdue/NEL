/******************************************************************************************
 *
 *	  		 PCI-6602 RAW Double-buffered Counter Driver for Matlab
 *
 * File: msdl.c Multi Spike Detector - Lite (no artificial ingredients)
 *
 *
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	///////////////////	
		...
		ret_status = mxGetPr(plhs[0]);
//		status = GPCTR_Watch (iDevice, ND_COUNTER_7, ND_BUFFER_SIZE , &watch_val);
		status = GPCTR_Watch (iDevice, ND_COUNTER_7, ND_AVAILABLE_POINTS , &watch_val);
		if (status < 0) 
			mexPrintf("ReadBuffer status = %d (trigger)\n", status);
		ret_status[0] = (double)watch_val;
	}
