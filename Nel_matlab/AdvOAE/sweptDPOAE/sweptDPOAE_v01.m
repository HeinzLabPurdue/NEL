...
cdd;
allCalibFiles= dir('*calib*raw*'); % TODO: Make FPL specific
...
t = stim.t;
nearfreqs = stim.nearfreqs; % for noise calc
...
while doneWithTrials == 0
    ...
    %pause(0.05);
     ...
     if k > stim.ThrowAway
            ...
            noise = mean(noise, 2);
            dpnf_full = db(noise.*VtoSPL);
            
            % weighted average to 9 center frequencies
