...
stim = struct( ...
    'fmax',16000, ...
    'fmin', 500, ...
    'speed', -1,  ... % oct/sec downsweep
    'diff', 50, ...  % Hz (Fprobe - 50 = Fsupp; Probe is higher)
    'Fs', 48828.125, ...
    'drop_Probe', 60, ... % for 40dB probe
    'drop_Supp', 40, ... % for 60dB suppressor
    'ThrowAway', 1, ...    % not saved
    'Averages', 50, ...    % not needed if SNR based
    'SNRcriterion', 6, ...
    'minTrials', 12, ...
    'maxTrials', 50, ...
    'buffdur', 0.1...   % on either side of sweep
    );
