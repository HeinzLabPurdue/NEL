...
FIG.edit.threshV2 = uicontrol(FIG.handle,'callback','CAP(''threshV2'');','style','edit','Units','normalized','position',[.18+.05 .54 .04 .04],'string',Stimuli.threshV2,'fontsize',12); % channel 2 threshold JMR nov 21
...
FIG.radio.chan_1 = uicontrol(FIG.handle,'callback','CAP(''chan_1'');', 'style','radio','Enable','on','Units','normalized','position',[.125*2 .235 .08 .03],'string','Chan 1','fontsize',12,'BackgroundColor','w','value',0);
FIG.radio.chan_2 = uicontrol(FIG.handle,'callback','CAP(''chan_2'');', 'style','radio','Enable','on','Units','normalized','position',[.125*2 .205 .08 .03],'string','Chan 2','fontsize',12,'BackgroundColor','w','value',0);
FIG.radio.Simultaneous = uicontrol(FIG.handle,'callback','CAP(''Simultaneous'');', 'style','radio','Enable','on','Units','normalized','position',[.125*2 .175 .08 .03],'string','Simultaneous','fontsize',12,'BackgroundColor','w','value',1);
if strcmp(interface_type,'ABR')
    ...
    FIG.push.Automate_Levels = uicontrol(FIG.handle,'callback','CAP(''Automate_Levels'');','style','pushbutton','Units','normalized',...
        'position',[.18 .726 .09 .09],'string','Auto Levels','fontsize',12,'fontangle','normal','fontweight','normal');
    FIG.push.close   = uicontrol(FIG.handle,'callback','CAP(''close'');','style','pushbutton','Units','normalized',...
