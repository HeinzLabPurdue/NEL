function firstSTIM=FFR_SNRenv_RunLevels(FIG,Stimuli,RunLevels_params, misc, FFR_SNRenv_Gating,...
    FFRnpts,interface_type, Display)

global NelData data_dir RP PROG
RP1=RP.activeX;
RP2=RP.activeX;
    ...
    for currStim = 0:2*RunLevels_params.nPairs
        ...
        while (bNoSampleObtained)
            if (invoke(RP2,'GetTagVal','BufFlag') == 1)
                ...
                set(FIG.ax.line3,'xdata',0.5,'ydata',maxFFRobs); drawnow;
                % zz 05nov11
                ...
                if (maxFFRobs <= critVal)
                    if  invoke(RP1,'GetTagVal','ORG') == mod(currStim,2)
                        %                 if double(invoke(RP1,'GetTagVal','ORG')) == mod(currStim,2)
                        %            if maxFFRobs <= critVal
                        bNoSampleObtained = 0;
                        ...
                        %              CAPdataAvg{attenIND} = CAPdataAvg{attenIND} +
                        %                     CAPdataReps{attenIND}(2*(currStim-1)+1,:);
                        %         end
...
%                                 end
                    ...
                    elseif invoke(RP1,'GetTagVal','ORG') == mod(currStim,2)
                ...
                %% Debugging
                fprintf('rejections %1.0f:: ORGval = %1.0f :: currStim= %1.0f :: MaxVal=%1.2f :: ugh %1.2f\n', ...
                    rejections, invoke(RP1,'GetTagVal','ORG') ,currStim,maxFFRobs, loopCount/max(currStim ,1));
                if loopCount/max(currStim ,1) >5
...
if (bAbort == 0)
    ...
    if ~strcmp(ButtonName,'No')
        ...
        make_FFR_Se_text_file(misc, Stimuli, PROG, NelData, comment, ...
