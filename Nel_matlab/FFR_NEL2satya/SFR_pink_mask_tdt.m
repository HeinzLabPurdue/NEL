function h_fig = SFR_pink_mask_tdt(command_str,eventdata)
...
if nargin < 1
    ...
    [FIG, FFR_SNRenv_Gating, Display]=SFR_pink_mask_tdt_loop_plot(FIG,Display,Stimuli,interface_type); ...
    SFR_pink_mask_tdt('fast');
    SFR_pink_mask_tdt('update_stim', 'SNR');
    SFR_pink_mask_tdt('update_stim', 'newStim');
...
elseif strcmp(command_str,'update_stim')
    ...
    switch eventdata
        case 'SNR'
            FIG.NewStim = 101;
            if get(FIG.bg.SNR.dB_m20, 'value')
                Stimuli.SNR_dB = -20;
            elseif get(FIG.bg.SNR.dB_m10, 'value')
                Stimuli.SNR_dB = -10;
            elseif get(FIG.bg.SNR.dB_0, 'value')
                Stimuli.SNR_dB = 0;
            elseif get(FIG.bg.SNR.dB_p10, 'value')
                Stimuli.SNR_dB = 10;
            elseif get(FIG.bg.SNR.dB_p120, 'value')
                Stimuli.SNR_dB = 120;
            end
    ...
    if updStimFlag
        ...
        xpr= xpr.*10^(Stimuli.stimAttenAdj_dB/20);
