% For Frequency Following Response to speech in pink noise masker
% 16 different realizations of pink noise saved as wav-file. 
% 256 reps of speech-in-noise stimuli are played using 16 reps for
% each of the 16 different pink-noise files. 
% Use SFR_pink_mask_tdt instead
% Uses online pink noise generator

function h_fig = SFR_pink_mask_SNRenv(command_str,eventdata)

% ge debug ABR 26Apr2004: replace "FFR" with more generalized nomenclature, throughout entire system.

global RP PROG FIG Stimuli FFR_Gating root_dir prog_dir Display 
%Stimuli.OLDDir
...
if nargin < 1
    ...
    [misc, Stimuli, RunLevels_params, Display, interface_type]=SFR_pink_mask_SNRenv_ins(); ...
    ...
    [FIG, FFR_SNRenv_Gating, Display]=FFR_SNRenv_loop_plot(FIG,Display,Stimuli,interface_type); ...
        %#ok<ASGLU>
    SFR_pink_mask_SNRenv('fast');
    SFR_pink_mask_SNRenv('update_stim', 'spl');
...
elseif strcmp(command_str,'update_stim')
    switch eventdata
        ...
        case 'list'
            ...
            FIG.popup.stims = uicontrol(FIG.handle,'callback', 'SFR_pink_mask_SNRenv(''update_stim'',0);','style', ...
