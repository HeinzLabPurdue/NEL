function varargout = DALinloop_NI_wavfiles_fast(varargin)
% SP: On 10/17/19. Trying to make inloop wav-file loading faster. 
% Original file: DALinloop_NI_wavfiles_fast
% % -----------------------------------------------------------------------
...
global root_dir cache_stim_data NelData % Used to find correct control 'rco' file.
...
if ((common.index == 1) || (length(static_bi.list) > 1))
    ...
    if ~strcmp(NelData.DAL.short_description, 'SynCap')
        % Originally, calls this function
        [data,~,rc] = nel_wavread(static_bi.list{min(ind,end)});
    else
        data= cache_stim_data{ind};
    end
    if (rc == 0)
