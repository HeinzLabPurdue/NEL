function [tmplt,DAL,stimulus_vals,units,errstr] = nel_rot_wavfile_template(fieldname,stimulus_vals,units)
%
...
if (exist('stimulus_vals','var') == 1)
   if (exist(stimulus_vals.Inloop.List_File,'file') ~= 0)
      ...
      %
      if (~isequal(playdur,prev_playdur) | ~isequal(min_period,prev_min_period))
...
%----------------------------------------------------------------------------------------
function errstr = check_DAL_params(DAL)
...
%if (isempty(DAL.Inloop.params.list) & isempty(DAL.Inloop.params.Rlist))
%   errstr = 'No wav-files to play!';
%end
...
tmplt.tag               = 'NELrot_wavfile_tmplt';
tmplt.IO_def = IO_def;
