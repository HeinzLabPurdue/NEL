function varargout = DALinloop_general_TN_delay(varargin)
%
...
if (common.index == 1) 
   ...
   case 'noise'
      ...
      % Modified by GE (22Nov2002) to use an .rco version that allows a delay:
       main_rco = [root_dir 'stimulate\object\noise_bursts.rco'];  % removed by GE (22Nov2002)
      %main_rco = [root_dir 'stimulate\object\noise_bursts_delay.rco'];
      % LQ 10/22/04 noise_bursts_delay.rco has cycusage problem, so
      % go back to noise_bursts.rco
      %params1.RiseFall  = static_bi.rise_fall;
      %params1.NoiseDelay = static_bi.noise_delay;   % added by GE (22Nov2002).
   case 'tone'
      params1.freq      = static_bi.main.freqs(1);
      % Modified by GE (07Jan2003) to use an .rco version that allows a delay:
      %  main_rco = [root_dir 'stimulate\object\tone_bursts.rco']; % removed by GE (07Jan2003)
      main_rco = [root_dir 'stimulate\object\tone_bursts_delay.rco'];
      params1.RiseFall  = static_bi.rise_fall;
      params1.ToneDelay = static_bi.tone_delay;   % added by GE (07Jan2003).
   ...
   if (isempty(sec_rco))
      ...
      if (isempty(specific.secondary.atten))
         nelerror('Background attenuation is not set!');
      end
