function varargout = DALinloop_wavfiles(varargin)
%
...
if (common.index == 1) 
   ...
   %%%%% Load rco to RP
   rconame = [root_dir 'stimulate\object\raw_samples.rco'];
   if (isempty(static_bi.Rlist))
...
if ((common.index == 1) | (length(static_bi.list) > 1))
   ...
   if (rc == 0)
      ...
      rc = load_raw_samples(1,data,sr) & (rc==1);
   end
...
if (((common.index == 1) | (length(static_bi.Rlist) > 1)) & (length(static_bi.Rlist) >0))
   ...
   if (rc == 0)
      ...
      rc = load_raw_samples(2,data,sr) & (rc==1);
   end
