function varargout = DALinloop_general_TN(varargin)
%
...
if (common.index == 1)
    ...
    static_bi = specific;
    if strcmpi(specific.main.source,'tone') &&  (specific.main.tone.bw > 0)
    ...
    elseif (~strcmpi(specific.main.source,'none'))    % modified to elseif by GE, 29Mar2002.
        % Tone and noise rate level
        if (strcmpi(specific.main.source,'noise'))
            source = 'Noise';
        else
            source = 'Tone';
        end
        ...
        if (nstim == 1)
            ...
            static_pi.XYprops.Tick   = 0:round(nlines/300)*10:nlines+1;
        else
        ...
        % Updated SP: 6 May 19
        if nlines<50
            static_pi.XYprops.Tick = 0:1:nlines;
        elseif nlines>30 && nlines<299
...
if ~strcmpi(specific.main.source,'none')    % if-statement added by GE, 29Mar2002.
