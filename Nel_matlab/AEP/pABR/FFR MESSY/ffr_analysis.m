function [out_env env_amp tfs_amp fm_sum fc_sum fm_p1 fc_p1 fc_sb noise] = ffr(x,isOLD)
% FFR
...
figure(1)
subplot(3,1,1)
plot(t,avgPO,'k',t,avgNP,'g');
title('Averaged Response vs. Time');
legend('Polarized','Non-polarized');
subplot(3,1,2)
plot(t,tfs);
hold on;
plot(smoothTFS.x,smoothTFS.f,'r');
title('Fine Structure (Non-Polarized - Polarized)');

subplot(3,1,3)
plot(t,env);
hold on;
plot(smoothENV.x,smoothENV.f,'r');
title('Envelope Response (Non-Polarized + Polarized)');
xlabel('time (s)');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Part 2: Hamming transformation of the cropped sections
% FIGURE 2

% USING CROPPED SIGNAL (strongest FFR response to AM signal)
t = t(x1:x2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Part 3: FFTs alternating polarities, ENV, and TFS
% FIGURE 3
...
yMAX = 1.1*max(NPamp(0.001*length(NPamp):0.999*length(NPamp)));

subplot(2,2,1);
plot(freq,NPamp);
axis([0 10 0 yMAX]);
title('FFT of response to Non-polarized signal');
...
plot(freq,POamp);
axis([0 10 0 yMAX]);
title('FFT of response to Polarized signal');
xlabel('Frequency (kHz)');

% FFT of Post Hamming env
ENVfft=fft(env,fftpts)/fftpts;
ENVamp = abs(2*ENVfft(1:(fftpts/2+1)));
ENVmag = 20*log10(ENVamp);
...
subplot(2,2,4)
plot(freq,TFSamp);
axis([fc-2*fm fc+2*fm 0 yMAX]);
title('FFT of Fine Structure');
xlabel('Frequency (kHz)');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Part 4: Calculating amplitudes of the FFT of hamming transformed TFS and ENV
%         Includes harmonics and side bands as necessary
% FIGURE 4

% FFT of Post HAMMING tfs and env
...
plot(freq,TFSamp,'r');
% axis([0 10 0 yMAX]);
axis([0 10 0 1.1*env_amp]);
title('FFT of Envelope and Fine Structure');
xlabel('Frequency (kHz)');
legend('Envelope','TFS');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FFT amplitude calculations
...
i = 2;
fm_sum = fm_amp(1);
lbl_txt = sprintf('\\leftarrow X= %6.3f Y= %f',freq(ENVamp==fm_amp(1)),fm_amp(1));  %labeling peaks
text(freq(ENVamp==fm_amp(1)),fm_amp(1),lbl_txt);

num_hmon = 1;
plot([0 10],[noise_floor noise_floor],'g-');
...
while fm_amp(i) > (noise_floor) && ((i+1) * fm) < fc && i <= iLimit
    ...
    lbl_txt = sprintf('\\leftarrow X= %6.3f Y= %f',freq(ENVamp==fm_amp(i)),fm_amp(i));
    text(freq(ENVamp==fm_amp(i)),fm_amp(i),lbl_txt);
    num_hmon = i;
    i = i + 1;
...
fc_amp(2) = max(TFSamp(find(freq>(fc-1.5*fm)&freq<(fc-0.5*fm)))); % side band (unsure on name)
fc_amp(3) = max(TFSamp(find(freq>(fc+0.5*fm)&freq<(fc+1.5*fm)))); % side band (unsure on name)

% adding the fc side bands if they are great enough magnitude
if fc_amp(2) > noise_floor || fc_amp(3) > noise_floor
    fc_sum = sum(fc_amp);
    for i = 1:3
        lbl_txt = sprintf('\\leftarrow X= %6.3f Y= %f',freq(TFSamp==fc_amp(i)),fc_amp(i));
        text(freq(TFSamp==fc_amp(i)),fc_amp(i),lbl_txt);
    end
    fc_p1 = fc_amp(1);
    fc_sb = fc_amp(2) + fc_amp(3);
else
    fc_sum = fc_amp(1);
    lbl_txt = sprintf('\\leftarrow X= %6.3f Y= %f',freq(TFSamp==fc_amp(1)),fc_amp(1));
    text(freq(TFSamp==fc_amp(1)),fc_amp(1),lbl_txt);
    fc_p1 = fc_amp(1);
    fc_sb = 0;
end
