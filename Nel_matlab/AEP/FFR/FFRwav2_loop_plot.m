...
fName=Stimuli.list;
%fName2=Stimuli.list2;   ??? where is this set???
...
% %% Stimulus2 menu
% FIG.popup.stims2 = uicontrol(FIG.handle,'callback', 'FFRwav(''update_stim'',''newStim'');','style','popup','Units','normalized','Userdata',Stimuli.filename,'position',[.4 .175 .425 .04],'string',{fName.name},'fontsize',12);
% FIG.push.prev_stim2 = uicontrol(FIG.handle,'callback','FFRwav(''update_stim'',''prevStim'');','style','pushbutton','Units','normalized','position',[.35 .175 .05 .04],'string','<<','fontsize',12,'fontangle','normal','fontweight','normal');
% FIG.push.next_stim2 = uicontrol(FIG.handle,'callback','FFRwav(''update_stim'',''nextStim'');','style','pushbutton','Units','normalized','position',[.825 .175 .05 .04],'string','>>','fontsize',12,'fontangle','normal','fontweight','normal');
