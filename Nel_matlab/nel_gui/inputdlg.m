function Answer=inputdlg(Prompt, Title, NumLines, DefAns, FigWidth, Resize)
%INPUTDLG Input dialog box. (Some fixed by AF - 11/29/01).
...
if (isstruct(FigWidth))  % AF 06/11/02: for backword compatibility.
   Resize = FigWidth;
   FigWidth = 300;
end
if (isstruct(Resize))  
...
% AF
CBString = 'ch=double(get(gcbf,''CurrentCharacter'')); if(ch==13), set(gcbf,''UserData'',''OK'');uiresume; end';
set(InputFig, 'keypress',   CBString);
% AF
