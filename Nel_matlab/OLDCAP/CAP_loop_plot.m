...
FIG.asldr.slider = uicontrol(FIG.handle,'callback','CAP(''slide_atten'');','style','slider','SliderStep',[0.00833 0.0833],'Enable','on','min',-120,'max',0,'Units','normalized','position',[.35 .075 .525 .04],'Value',-Stimuli.atten_dB);
...
FIG.statText.status =  uicontrol(FIG.handle, 'callback', '', 'style', 'text','Units','normalized', 'position', [.05 .9 .3 .03], 'string', 'STATUS: free running...','fontsize',12,'BackgroundColor','w','horizontalalignment','left');   % added by GE 17Jan2003.
...
FIG.radio.right  = uicontrol(FIG.handle,'callback','CAP(''right'');','style','radio','Enable','on','Units','normalized','position',[.125 .205 .08 .03],'string','Right Ear','fontsize',12,'BackgroundColor','w','value',0);
FIG.radio.both   = uicontrol(FIG.handle,'callback','CAP(''both'');', 'style','radio','Enable','on','Units','normalized','position',[.125 .175 .08 .03],'string','Both Ears','fontsize',12,'BackgroundColor','w','value',1);

FIG.push.run_levels = uicontrol(FIG.handle,'callback','CAP(''run_levels'');','style','pushbutton','Units','normalized','position',[.1 .726 .125 .09],'string','Run levels...','fontsize',12,'fontangle','normal','fontweight','normal');
