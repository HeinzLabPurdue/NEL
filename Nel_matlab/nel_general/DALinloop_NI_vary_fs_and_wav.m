function varargout = DALinloop_NI_vary_fs_and_wav(varargin)

% SP on 14 Oct 19: To read wav-files in a list at play each wav-file at
% some sampling rate. Wav-files can be of different duration, also single
% or both polarities, dB SPL for each wav-file. So each condition will iterate through 
% [wav_file_name | sampling rate | polarity | stim-off duration | attenuation (computed from dB SPL)] 
% Original File: DALinloop_NI_wavfiles
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
...
if (nargin == 0)    % "clean-up" only (at end of data_acquisition_loop)
    ...
    rc_d2a = check_error_codes(statCodes, retVals, 37); %#ok<*NASGU>
    return;
elseif (nargin >=1)
    common = varargin{1};
end
...
if (common.index == 1)  % first picture
    ...
    if (min(length(static_bi.list),length(static_bi.attens)) ~= 1 && length(static_bi.list) ~= length(static_bi.attens))
        error('Inconsistent number of attenuations (%d) and file names (%d)', ...
    ...
    if (length(static_bi.attens) > 1)
        ...
        if (nstim > 1)
            ...
            for ii = 1:specific.repetitions
                for jj = 1:nstim
                    [~, fname] = fileparts(static_bi.list{jj});
                    var_labels{counter} = fname;
                    if (~isempty(static_bi.Rlist))
                        [~, fname] = fileparts(static_bi.Rlist{jj});
            ...
            if nlines<300
                static_pi.XYprops.Tick = 0:10:nlines+1;
            else
                lineNUMres_approx= nlines/30;
                static_pi.XYprops.Tick = 0:10*round(lineNUMres_approx/10):nlines+1;
            end
...
if ((common.index == 1) || (length(static_bi.list) > 1))
    ...
    [data,~,rc] = nel_wavread(static_bi.list{min(ind,end)});
    if (rc == 0)
...
if (((common.index == 1) || (length(static_bi.Rlist) > 1)) && (~isempty(static_bi.Rlist)))
    ...
    [data,~,rc] = nel_wavread(static_bi.Rlist{min(ind,end)});
    if (rc == 0)
...
if (current_nSamples > required_nSamples) % Waveform needs to be truncated.
    ...
    % LQ 03/30/05 comment the following 3 lines so that this warning will not show on the nelerror
    % window all the time which makes the user insensitive for the real error
    %       beep off
...
elseif (current_nSamples < required_nSamples) % Waveform needs to be "repeated".
    ...
    % LQ 03/30/05 comment the following line so that this warning will not show on the nelerror
    % window all the time which makes the user insensitive for the real error
...
rc_d2a = 0;
if ( (~isempty(find(statCodes~=0, 1))) || (~isempty(find(retVals~=0, 1))) )
    rc_d2a = -1;
    fprintf('\tDALInloop Location: %d\n',lineNum);
