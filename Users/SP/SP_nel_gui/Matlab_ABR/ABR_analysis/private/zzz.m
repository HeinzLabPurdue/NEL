...
if ischar(abr_Stimuli.abr_pic)
    [pic] = ParseInputPicString_V2(abr_Stimuli.abr_pic); 
else 
    pic=abr_Stimuli.abr_pic;
end
num=length(pic);
...
CalibFile= dir(sprintf('p%04d_calib*raw*', abr_Stimuli.cal_pic));
CalibFile= CalibFile.name;
% CalibFile  = sprintf('p%04d_calib', abr_Stimuli.cal_pic);
