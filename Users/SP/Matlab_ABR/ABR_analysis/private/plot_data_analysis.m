...
axes(han.amp_panel); plot(0,0,'-w');
axes(han.lat_panel); plot(0,0,'-w');
axes(han.z_panel); plot(0,0,'-w'); 
axes(han.peak_panel); plot(0,0,'-w');
...
axes(han.z_panel)
if isempty(reff)
	set(han.z_panel,'Box','on','XLim',[min(spl)-10 max(spl)+10],'YLim',[0 max([eps data.z.score])*1.1],...
