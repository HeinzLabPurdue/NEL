function [tmplt,DAL,stimulus_vals,units,errstr] = EHINvS_reBFi_template(fieldname,stimulus_vals,units)
...
if (exist('stimulus_vals','var') == 1)
   ...
   if ~BADstim   
      ...
      if (exist(stimulus_vals.Inloop.Compiled_FileName,'file') ~= 0)
         ...
         if BASELINE_Fs~=BASENOISE_Fs
            nelerror('In EHINvSreBFi_template: BASE sampling rates dont match between EH and NOISE!!');
         end
      else
         Llist = {};
         Rlist = {};
   ...
   OctShifts=[-.75 -.50 -.25 -.15 -.05 0 .05 .15 .25 .50];
...
%    %    Levels_dBSPL=stimulus_vals.Inloop.Max_Sig_Level+[-30 -15 0];  % THREE LEVELS
%    
   ...
   DAL.short_description   = 'EHvSrBFi';
...
IO_def.Inloop.Max_Sig_Level  =  {85 'dB SPL'       [-50    150]   0  0}; 
