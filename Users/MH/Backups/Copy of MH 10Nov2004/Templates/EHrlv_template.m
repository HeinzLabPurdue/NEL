...
%   - kep same params as for EH_reBF, but hardwired OCTshift to 0
%
...
str{1} = sprintf('RLF %d reps:', p.repetitions);
str{1} = sprintf('%s File ''%s''', str{1},file);
str{1} = sprintf('%s (%s),', str{1}, stimulus_vals.Mix.File);

if (length(p.attens) > 1)
   str{1} = sprintf('%s @ %1.1f - %1.1f dB Attn. (%1.1f dB steps)', str{1}, p.attens(1), p.attens(end), diff(p.attens(1:2)));
else
...
IO_def.Inloop.dBstep_Atten  = { 1           'dB'    [1    120]      };
