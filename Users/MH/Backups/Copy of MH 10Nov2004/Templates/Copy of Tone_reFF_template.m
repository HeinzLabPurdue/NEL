function [tmplt,DAL,stimulus_vals,units,errstr] = Tone_reFF_template(fieldname,stimulus_vals,units)
% Modified by M.Heinz 12Nov2003, from nel_rate_level_template.m
%    Places tone at a pre-dtermined frequency, based on BF 
%
...
if (exist('stimulus_vals','var') == 1)
   ...
   if ~isempty(stimulus_vals.Inloop.BaseFrequency) 
      ...
      Inloop.params.Condition.BaseFrequency_kHz                  = .040;
   end
   ...
   Inloop.params.main.tone.freq              = Inloop.params.Condition.BaseFrequency_kHz*1000;
...
%    disp(sprintf('Stim Freq: %.2f kHz; Atten: %.1f dB',Inloop.params.main.tone.freq,Inloop.params.main.attens))
   ...
   if(Inloop.params.main.tone.freq ~= CalcFixedFreq*1000)
...
if (isequal(fieldname,'Inloop'))
   ...
   elseif DAL.Inloop.params.CalibPicNum==0
      ...
      if (DAL.Inloop.params.main.tone.freq==40)
