...
persistent nstim ur_Hz  % MH:10Nov2004
         ...
         for ii = 1:specific.repetitions
            for jj = 1:nstim
               ...
               if (length(static_bi.attens) == 1)
                  var_labels{counter} = sprintf('%s, %.3f octs, %.1f dB',static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens);
               else
                  var_labels{counter} = sprintf('%s, %.3f octs, %.1f dB',static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens(jj));
               end
               if (ii > 1)
                  var_labels{counter} = [var_labels{counter} ' (Repetition #' int2str(ii) ')']; 
               end
...
if ((common.index == 1) | (length(static_bi.list) > 1) | (length(static_bi.updateRate_Hz) > 1))
   ...
   if (length(static_bi.list) > 1)
      ...
      stim_info.playback_sampling_rate(1) = ur_Hz;
   else                                           
