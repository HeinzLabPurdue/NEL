...
if (index > 0)
    ...
    for i = 1:length(spikes.times) % # of channels
        ...
        if nstim == 0 || nstim ==1
            NumDrivenSpikes=sum(yd(~isnan(yd))*rate_params.stim_dur)*ones(1,3);
        else
            ydINDs=find(~isnan(yd));
            Cond_nspikes=zeros(1,nstim);
            for condIND=1:nstim
                cond_ydINDs=ydINDs(mod(ydINDs-1,nstim)==(condIND-1));
...
spikes_fig(nCh);
[~, rate_params] = spikes_fig('reset_axes',[],rate_params,plot_info);