function parameters = handsy_init(command, argument, g0, nws, sr, bgs)
%handsy_init Interactive synthesis parameter tool
%   I = handsy_init allows the user to interactively specify the control
...
%
%   handsy_init(DUR, NFC, G0, NWS, SR, BGS) additionally allows the user to
...
%   Based on the FORTRAN program handsy_init.FOR in Klatt (1980) "Software for
...
if ~ischar(command)
    ...
    h = uicontrol('Parent', cf, ...
        ...
        'CallBack', 'handsy_init(''label'',''v'')', ...
        'String', 'v');
    set(h, 'Units', 'normalized')
    h = uicontrol('Parent', cf, ...
        'Position', [ncol*bsize+bsize/2 pos(4)/2+bsize 2*bsize/3 2*bsize/3], ...
        'TooltipString', 'raise value', ...
        'CallBack', 'handsy_init(''label'',''^'')', ...
        'String', '^');
    set(h, 'Units', 'normalized');
    pos = get(h, 'Position');
    ...
    switch command
        ...
        case {'close','play'}
            ...
            d = get(findobj(figHan,'Tag', 'deltat'), ...
                'UserData'); % update rate
            h = findobj(figHan, 'Tag', 'parameter');
            ...
            if strcmp(command, 'close')
                set(figHan, 'UserData', pars)
                
%                 uiresume(figHan) % return control to main
                % program
            else
                fs = pars.sr;
                n = parmcont(pars);
        ...
        case 'contour'
            ...
            xl = text(p(1), p(2), int2str(pos(1)), ...
                ...
                'Tag', 'xlabel', ...
                'ButtonDownFcn', 'handsy_init(''label'',''x'')');
            h = ylabel('');
            p = get(h, 'Position');
            ...
            yl = text(p(1), p(2), int2str(pos(2)), ...
                ...
                'ButtonDownFcn', 'handsy_init(''label'',''y'')');
            ...
            u = [double(s.nodes), find(pos(1) == xdata), d, s.minval, ...
                s.maxval, double(xl), double(yl), NaN, double(s.contour), s.term0];
        ...
        case 'delete'
            ...
            set(cbo, 'BackgroundColor', [.75 .75 .75], 'Callback', ...
                sprintf('handsy_init(''draw'',''%s'')', argument), ...
                'UserData', s)
            redrawplot
