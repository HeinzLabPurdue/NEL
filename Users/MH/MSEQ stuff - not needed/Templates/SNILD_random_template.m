function [tmplt,DAL,stimulus_vals,units,errstr] = SNILD_random_template(fieldname,stimulus_vals,units)
%

% AF 4/11/02
% Modified by SMC, 4/5/04

global signals_dir schase_list_bf_str schase_list_startrep schase_list_nreps 
% schase_list_startrep and schase_list_nreps: globals taht need to be accessible to the
% snild_randomcarrier.m list file. schase_list_bf_str is leftover from previous templates, and while no
...
if (exist('stimulus_vals','var') == 1)
   ...
   schase_list_startrep = stimulus_vals.Inloop.StartRep;
   schase_list_nreps=stimulus_vals.Inloop.Presentations;
   if stimulus_vals.Inloop.Frequency > useupshiftfreq
      List_File = [signals_dir 'Lists\schase\snild_randomcarrier_us.m' ];
   else
      List_File = [signals_dir 'Lists\schase\snild_randomcarrier.m' ];
   end
...
%             end
   ...
   Inloop.params.Presentations         = stimulus_vals.Inloop.Presentations;
%    Inloop.params.repetitions           = 1;
   ...
   DAL.short_description   = 'SNRC';  % Stands for Spectral Notch, Random Carrier
...
p = DAL.Inloop.params;
str{1} = sprintf('Random carrier ILD/SN resampled to %s ', schase_list_bf_str);
if (~isempty(p.attens));
...
IO_def.Inloop.Presentations     =  { 4                                ''     [1    Inf]      };
IO_def.Inloop.StartRep          =  { 1                                ''     [1    100]      };
