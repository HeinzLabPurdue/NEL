function [tmplt,DAL,stimulus_vals,units,errstr] = spont_template(fieldname,stimulus_vals,units)

% edited from mseq_template.m written by schase
% dma 1/13/05
% this template will record an arbitrarily long sample of spontaneous rate 
% will need to edit later to allow for stimulus playing
...
if (exist('stimulus_vals','var') == 1)
   SPONTsigdir=[signals_dir 'dma' filesep 'spont'];
   [successfulbuild,data,full_dur,act_frame_dur]=build_spont(stimulus_vals.Inloop.Frequency,...
      stimulus_vals.Inloop.FrameDuration, stimulus_vals.Inloop.Order,SPONTsigdir);
   if length(data)>7000000  %%%THIS SHOULD BE ARBITRARILY LONG - NEED TO CHANGE (*DMA*)
      nelwarn('Make sure the buffer in .rco is big enough!');
   end
   if successfulbuild
      ...
      stimfile=[SPONTsigdir filesep 'SPONT' freqstr 'kHz_f' framedurstr '_o' orderstr '.wav']; %CURRENTLY, THIS CAN SPECIFY NO STIMULUS (*DMA*)
