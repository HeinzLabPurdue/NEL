function [tmplt,DAL,stimulus_vals,units,errstr] = schase_itdild_template(fieldname,stimulus_vals,units)
%

% AF 4/11/02  % SC 6/14/02
global signals_dir
...
%   schase_list_bf_str = num2str(stimulus_vals.Inloop.Frequency);
   List_File = [signals_dir 'Lists\schase\itdild.m' ];
...
p = DAL.Inloop.params;
%str{1} = sprintf('ILD resampled to %s (%d,%d files) ', schase_list_bf_str, length(p.list), length(p.Rlist));
str{1} = sprintf('ITDILD (%d,%d files) ', length(p.list), length(p.Rlist));
if (~isempty(p.attens));
