function [successfulbuild,y,act_dur_ms,act_frame_dur]=build_spont(freq,frame_dur,order,varargin)
...
fs=97656.25;
successfulbuild=1;
truncd_frame_dur=round(frame_dur*2*freq)/2/freq; % round to nearest half-cycle
frame_dur_samps=floor(truncd_frame_dur/1000*fs);
dur_samps=2^order*frame_dur_samps;
act_frame_dur=frame_dur_samps/fs*1000;
...
if ~isequal(sigdir,'')
   filename=[sigdir filesep 'spont' freqstr 'kHz_f' framedurstr '_o' orderstr '.wav'];  %SHOULD NOT PRESENT ANY STIMULUS (*DMA*)
else
   filename=['spont' freqstr 'kHz_f' framedurstr '_o' orderstr '.wav'];
end
