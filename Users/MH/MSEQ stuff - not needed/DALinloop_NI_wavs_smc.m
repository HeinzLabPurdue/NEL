function varargout = DALinloop_NI_wavs_smc(varargin)
...
% Adapted 4/2/04 by SMC from GE's code of 06oct2003.  
% Essentially, I'm using this to perform sampling rate adjustments online for my random carrier stimuli.  
% Because I am not using frozen noise, specific repetitions will always be 1, even though actual repetions 
% is, of course, number of presentations / number of SN/ILD combinations (25).  

% Had to rewrite refit_wavform below because it assumes continuity in the stimulus, such that the stimuli
% can be seamlessly appended.  I add ramps to the beginning and end to avoid click-y perceptions.

% Adapted by GE 26Jul2002 from 'DALinloop_wavfiles' (AF 9/22/01).
%  Further modifications: GE 03oct2003, 04oct2003, 06oct2003.

% 6/24/04 - Put in a debug to extract and save the files as they are generated.  Then commented it out, so
% the program is the same as on south chamber...   SMC
...
if ((common.index == 1) | (length(static_bi.list) > 1))
   ...
   [data,sr,rc] = nel_wavread_smc(static_bi.list{min(ind,end)});
   if (rc == 0)
...
if (((common.index == 1) | (length(static_bi.Rlist) > 1)) & (length(static_bi.Rlist) >0))
   ...
   [data,sr,rc] = nel_wavread_smc(static_bi.Rlist{min(ind,end)});
   if (rc == 0)
...
% Start debug SMC
% varname=sprintf('rc_%d',varargin{1}.index);
% varstr=sprintf('%s = samplesMatrix;',varname);
% eval(varstr);
% curdir=pwd;
% cd 'D:\Users\schase';
% if varargin{1}.index~=1
%    savestr=sprintf('save rceval %s -append;',varname);
% else
%    savestr=sprintf('save rceval %s;',varname);
% end
% eval(savestr);
% cd(curdir);
% End debug SMC
...
function out_wave = refit_waveform(in_wave)
   ...
   while (current_nSamples ~= required_nSamples)
      if (current_nSamples > required_nSamples) % Waveform needs to be truncated.
         in_wave = in_wave(1:required_nSamples); % No problem, adding ramps later.
      else % Waveform needs to be "repeated" or zero filled.
         if isequal(NelData.DAL.Inloop.params.Expand,'No') % Truncate duration to nearest msec, zero fill
            NelData.DAL.Gating.Duration = NelData.DAL.Gating.Duration - ceil((requried_nSamples-current_nSamples)/ur_Hz*1000);
            nelwarnstr=sprintf('In function ''DALinloop_NI_wavs_smc'': decreased duration to %d',...
               NelData.DAL.Gating.Duration);
            nelwarn(nelwarnstr);
         else % Repeat stimulus
            rf_time_sec=NelData.DAL.Gating.Rise_fall_time / 1000;
            rf_time_samples=ceil(ur_Hz*rf_time_sec);
            ramp=linspace(0,1,rf_time_samples)';
            in_wave(end-rf_time_samples+1:end)=ramp(end:-1:1).*in_wave(end-rf_time_samples+1:end);
            in_wave=[in_wave;ramp.*in_wave(1:rf_time_samples);in_wave(rf_time_samples+1:end)];
         end
      end
      stimOn_sec = NelData.DAL.Gating.Duration / 1000;
      ur_Hz = static_bi.updateRate_Hz;
