...
line_time_stamp=TRIGstart_time;
debug_index=1;
...
%% For debugging
line_times_start = NaN*zeros(1,nlines); %%MH/GE
line_times_start(1)=0;
msdl_counter = 1;
msdl_times = NaN*zeros(2,10000); %%X ([before,after])
msdl_lines = NaN*zeros(1,10000); %%X
msdl_trigs = NaN*zeros(1,10000); %%X
DALinloop_counter = 1;
DALinloop_times = NaN*zeros(2,nlines); %%X
DALinloop_lines = NaN*zeros(1,nlines); %%X
%%
...
%    end
   ...
   %% GE debug 
   [trig_state_debug count_down_debug] = TRIGget_state;
   index=MAXtrigs-count_down_debug+1; % GE debug

%   if (trig_state_debug~=1)
      %% For debugging MSDL
      msdl_times(1,msdl_counter) = etime(clock,TRIGstart_time)-(index-1)*DAL.Gating.Period/1000; %%X
      msdl_lines(msdl_counter) = index;%%X
...
%       end
      ...
      %% For debugging
      msdl_times(2,msdl_counter) = etime(clock,TRIGstart_time)-(index-1)*DAL.Gating.Period/1000; %%X
      msdl_lines(msdl_counter) = index;%%X
      msdl_counter = msdl_counter+1;   %% 
      %% 
      %% For debugging
      msdl_trigs(msdl_counter) = trig_state_debug;%%X
      %   end
...
%    end
   ...
   if ((index-Nbadstim>last_stimsent_index)&(trig_state == 2))  % Trigger pulse switched to off
      ...
      if (index-Nbadstim < nlines)
         ...
         %% For debugging
         DALinloop_times(1,DALinloop_counter) = etime(clock,TRIGstart_time)-(index-1)*DAL.Gating.Period/1000; %%X
         DALinloop_lines(DALinloop_counter) = index;%%X
         %% 
         
         [stim_info(index+1)] = call_user_func(DAL.Inloop.Name,common,DAL.Inloop.params);

         %% For debugging
         DALinloop_times(2,DALinloop_counter) = etime(clock,TRIGstart_time)-(index-1)*DAL.Gating.Period/1000; %%X
         DALinloop_lines(DALinloop_counter) = index;%%X
         DALinloop_counter = DALinloop_counter+1;   %% 
         %% 
