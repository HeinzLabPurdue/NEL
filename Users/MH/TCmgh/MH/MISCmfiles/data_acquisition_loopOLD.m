...
end_of_loop_flag = 0;
index = 1;
inloop_time_stamp = clock;
last_endline_display  = clock;
%%%%%%%%%%% MAIN PRESENTATION LOOP %%%%%%%%%%%%%%%
%% ADD time check for the off period!!!
while (end_of_loop_flag == 0)
   if (etime(clock,inloop_time_stamp) > 0.7*trig_period)
      nelerror(['System is too busy to manage the data acquisition. ' ...
   ...
   if ((trig_state == 0) & (count_down == nlines))
      ...
      if (((index - (nlines-count_down+1) < 0) | (index - (nlines-count_down+1) >1)) & ~(nlines == index))
         nelerror(sprintf( ...
   ...
   if ((trig_state == 2) & (trig_prev_state == 1)) % Trigger pulse switched to off
      trig_offtime = clock;
   ...
   elseif (((trig_state ~= 2) & (trig_prev_state == 2)) | ...
         (index > 1 & etime(clock,last_endline_display)>1*trig_period)) 
      % Trigger pulse switched to on or last line ended.
      number_of_presented_lines = index-1;
      last_endline_display = clock;
      call_user_func(contPlotParams.func,{},contPlotParams,[],[],1:nChannels); % Allow contplot to set all the data points.
      call_user_func(endLinePlotParams.func,index-1,endLinePlotParams);
