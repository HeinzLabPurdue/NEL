function varargout = DALinloop_NI_HTCi_wavfiles(varargin)
% Modified by SK from DALinloop_NI_SCCi_wavefiles (July 27, 2010)
% Modification removed multiple checks on static_bi structure
% Code now is applicable only to HTC and HTC related templates
% Major modifications on lines 107-141
...
if (common.index == 1)  % first picture
   ...
   % SK:July 27, 2010 PilotHTC interleaving conditions
   if (length(static_bi.Rattens) > 1) & (length(static_bi.list) > 1)
      %    if (nstim > 1)
      ...
      for ii = 1:specific.repetitions
         for jj = 1:nstim
            %             if (length(static_bi.Rattens) == 1)
            %                [pathstr,name,ext,vers]=fileparts(static_bi.list{jj});
            %                var_labels{counter} = sprintf('(%d)Shift %s, %.2f, .2f (Hz)\n%s@%.1f dB+Noise@ %.1f dB\n', ...
            %                   jj,static_bi.Used.ShiftType,static_bi.Used.percentShifts(jj),static_bi.Used.shifts_hz(jj), ...
            %                   name,static_bi.attens,static_bi.Rattens(jj));
            %             else
            if (length(static_bi.Used.ShiftType) == 1)
               [~,name_TEMP,~]=fileparts(static_bi.list{jj});
               var_labels{counter} = sprintf('(%d)Shift %s, %.2f, %.2f (Hz)\n%s@%.1f dB+Noise@ %.1f dB', ...
                  jj,static_bi.Used.ShiftType,static_bi.Used.percentShifts(jj),static_bi.Used.shifts_Hz(jj), ...
                  name_TEMP,static_bi.attens(jj),static_bi.Rattens(jj));
            else
               [~,name_TEMP,~]=fileparts(static_bi.list{jj});
               var_labels{counter} = sprintf('(%d)Shift %s, %.2f, %.2f (Hz)\n%s@%.1f dB+Noise@ %.1f dB', ...
                  jj,static_bi.Used.ShiftType{jj},static_bi.Used.percentShifts(jj),static_bi.Used.shifts_Hz(jj), ...
                  name_TEMP,static_bi.attens(jj),static_bi.Rattens(jj));
            end
            if (ii > 1)
   ...
   elseif (length(static_bi.attens) > 1) & (length(static_bi.list) > 1)
      static_pi.var_name = 'Condition';
      static_pi.var_unit = '';
      static_pi.var_frmt = '%s';
      var_labels = cell(1,nlines);
      ...
      for ii = 1:specific.repetitions
         for jj = 1:nstim
            ...
            %             else
            [~,name_TEMP,~]=fileparts(static_bi.list{jj});
            var_labels{counter} = sprintf('(%d) %s @%.1f dB',jj,name_TEMP,static_bi.attens(jj));
            %             end
            if (ii > 1)
               var_labels{counter} = [var_labels{counter} ' (Rep #' int2str(ii) ')']; 
            end
