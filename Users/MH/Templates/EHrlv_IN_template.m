function [tmplt,DAL,stimulus_vals,units,errstr] = EHrlv_IN_template(fieldname,stimulus_vals,units)
...
if (exist('stimulus_vals','var') == 1)
   ...
   Inloop.params.Condition.Noise_Attenuation           = stimulus_vals.Inloop.Noise_Attenuation;
...
str{1} = sprintf('RLF_IN %dreps:', p.repetitions);
...
str{1} = sprintf('%s noise@ %.0f dB Attn', str{1}, stimulus_vals.Inloop.Noise_Attenuation);
...
IO_def.Inloop.Noise_Attenuation = { []           'dB'    [0    120]      };
