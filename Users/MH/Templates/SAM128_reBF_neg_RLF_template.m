function [tmplt,DAL,stimulus_vals,units,errstr] = SAM128_reBF_neg_RLF_template(fieldname,stimulus_vals,units)
...
if (exist('stimulus_vals','var') == 1)
    ...
    DAL.short_description   = 'SAM128_neg_rlf';
...
OutFolder= [NelData.File_Manager.dirname 'Signals' filesep 'SAM' filesep sprintf('T%.0f_U%.0f', NelData.File_Manager.track.No, NelData.File_Manager.unit.No) filesep];
if ~isdir(OutFolder) % means files have not been created for this track/unit
    mkdir(OutFolder);
end

ModFreq= 128;
fName= create_SAM_reBF(OutFolder, NelData.File_Manager.unit.BF*1e3, ModFreq, 'neg');
...
%% Gating Section
%%%%%%%%%%%%%%%%%%%%
IO_def.Gating.Duration             = {500       'ms'    [20 4000]};
