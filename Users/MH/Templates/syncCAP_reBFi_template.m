...
if (exist('stimulus_vals','var') == 1)
   ...
   if ~BADstim   
      ...
      if isempty(dir(fullfile(EHsignals_dir,filename)))
         ...
         audiowrite(fullfile(EHsignals_dir,filename),stim,Fs)
      end
      ...
      if (exist(stimulus_vals.Inloop.Compiled_FileName,'file') ~= 0)
         ...
         if BASELINE_Fs~=BASENOISE_Fs
            nelerror('In EHINvNreBFi_template: BASE sampling rates dont match between EH and NOISE!!');
         end
      else
         Llist = {};
         Rlist = {};
      ...
      if strcmp(stimulus_vals.Inloop.use_TONE,'yes')
         ...
         if isempty(dir(fullfile(EHsignals_dir,TONEfilename)))
            ...
            audiowrite(fullfile(EHsignals_dir,TONEfilename),tone,BASELINE_Fs)
         end
      end
