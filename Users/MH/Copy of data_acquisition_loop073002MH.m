...
if (~isstruct(stim_info))
   ...
   MAX_pre_nlines=ceil(1.2*nlines);  %% For memory pre-allocation
...
while (end_of_loop_flag == 0)
   ...
   if ((trig_state == 0) & (count_down == MAXtrigs)) | (index-Nbadstim>nlines)
...
SBset([7 7],[0 0]);
if (end_of_loop_flag)
   stim_info = stim_info(1:index-1);  %Remove extra pre-allocated lines
else  % user (or error) break, not all the stimuli were fully presented!
   stim_info = stim_info(1:index);  %Store all lines presented at all, good or bad
end
