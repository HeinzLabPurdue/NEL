...
if nargin < 1
    ...
    abr_FIG.parm_txt(1)  = text(.8,.55,num2str(abr_Stimuli.cal_pic),   'fontsize',10,'color','b','horizontalalignment','right','buttondownfcn','abr_analysis(''stimulus'',1);');
    abr_FIG.parm_txt(2)  = text(.8,.45,num2str(abr_Stimuli.abr_pic),   'fontsize',10,'color','b','horizontalalignment','right','buttondownfcn','abr_analysis(''stimulus'',2);');
    abr_FIG.parm_txt(3)  = text(.8,.37,num2str(abr_Stimuli.start_resp),'fontsize',10,'color','b','horizontalalignment','right','buttondownfcn','abr_analysis(''stimulus'',3);');
    abr_FIG.parm_txt(4)  = text(.8,.32,num2str(abr_Stimuli.end_resp),  'fontsize',10,'color','b','horizontalalignment','right','buttondownfcn','abr_analysis(''stimulus'',4);');
    abr_FIG.parm_txt(5)  = text(.8,.24,num2str(abr_Stimuli.start_back),'fontsize',10,'color','b','horizontalalignment','right','buttondownfcn','abr_analysis(''stimulus'',5);');
    abr_FIG.parm_txt(6)  = text(.8,.19,num2str(abr_Stimuli.end_back),  'fontsize',10,'color','b','horizontalalignment','right','buttondownfcn','abr_analysis(''stimulus'',6);');
    abr_FIG.parm_txt(7)  = text(.8,.09,num2str(abr_Stimuli.scale),     'fontsize',10,'color','b','horizontalalignment','right','buttondownfcn','abr_analysis(''stimulus'',7);');
    
    abr_FIG.dir_txt = text(.8,.65,abr_Stimuli.dir,'fontsize',10,'color','r','horizontalalignment','right','buttondownfcn','abr_analysis(''directory'');','Interpreter','none');
    ...
    hold on;
    for i = 2:10
        eval(['abr_FIG.abrs.abr' num2str(i) ' = plot(0,0,''b-'',''LineWidth'',2,''Visible'',''off'');']);
    end
    abr_FIG.ax2.xlab = xlabel('Time (msec)','FontSize',14);
