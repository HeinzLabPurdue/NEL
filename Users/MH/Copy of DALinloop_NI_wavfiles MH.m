...
if (nargin == 0)    % "clean-up" only
   static_bi = [];
   static_pi = [];
   clear static_bi static_pi;
%    [statCodes retVals] = d2a(0);  % Initialize NI 6052 board (see file "d2a.c")
%    rc_d2a = check_error_codes(statCodes, retVals, 36);
   return;
elseif (nargin >=1)
   common = varargin{1};
end
...
disp(sprintf('Index=%d',common.index)) % ge debug


% Choose action based on call-mode ('common.index'):
if (common.index == 0)  % initialization
...
if mod(common.index,2)==3  % ge debug
   [statCodes retVals] = d2a(0);
   rc_d2a = check_error_codes(statCodes, retVals, 243);
end

% Load the samples data to the NI board (has to be done anew for every line):
...
elseif (~isempty(Chan0data))    % load channel 0 only.
   ...
   [statCodes retVals] = d2a(1, static_bi.updateRate_Hz, samplesMatrix);
   rc_d2a = check_error_codes(statCodes, retVals, 235);
else     % initialize D/A.
   samplesMatrix = [];
   [statCodes retVals] = d2a(0);
   rc_d2a = check_error_codes(statCodes, retVals, 239);
end
...
function out_wave = refit_waveform(in_wave)
   ...
   required_nSamples = ur_Hz*stimOn_sec;
...
function out_wave = window_waveform(in_wave)
   ...
   ramp_nSamples = ur_Hz * rf_time_sec;
