...
%    rc_d2a = check_error_codes(statCodes, retVals, 51);
   ...
   %  distinct outputs):
   nstim = max([length(static_bi.list),length(static_bi.attens),length(static_bi.updateRate_Hz)]);  % MH:10Nov2004: Adding UpdateRate_Hz to possible multiple values
      
   % Check to make sure that # of attenuations matches number of files, if using list.
   if length(static_bi.list)~=1 & length(static_bi.list)~=nstim
      error(sprintf('Inconsistent number of file names (%d)',length(static_bi.list)));
   end
   if length(static_bi.attens)~=1 & length(static_bi.attens)~=nstim
      error(sprintf('Inconsistent number of attenuations (%d)',length(static_bi.attens)));
   end
   if length(static_bi.updateRate_Hz)~=1 & length(static_bi.updateRate_Hz)~=nstim
      error(sprintf('Inconsistent number of NIboard update rates (%d)',length(static_bi.updateRate_Hz)));
   end
   ...
   if (length(static_bi.attens) > 1) & (length(static_bi.list) == 1) & (length(static_bi.updateRate_Hz)== 1)  % MH:10Nov2004 Generalizing
      % Rate-level function for 1 stimulus
   ...
   elseif (length(static_bi.attens) == 1) & (length(static_bi.list) > 1) & (length(static_bi.updateRate_Hz)== 1)  % MH:10Nov2004 Generalizing
      % Pure file rotation with rest of params fixed      
      if (nstim > 1)
         ...
         for ii = 1:specific.repetitions
            for jj = 1:nstim
               ...
               if (ii > 1)
                  var_labels{counter} = [var_labels{counter} ' (Rep #' int2str(ii) ')']; 
               end
               counter = counter+1;
            end   
         end
         ...
         % PST
         static_pi.var_name = 'Rep #';
      end
   elseif (length(static_bi.updateRate_Hz) > 1)  % MH:10Nov2004 EH_SCC interleaving conditions
      % General EH_SCC interleaving conditions (may have multiple attens and/or files
      if (nstim > 1)
         static_pi.var_name = 'Condition';
         static_pi.var_unit = '';
         static_pi.var_frmt = '%s';
         var_labels = cell(1,nlines);
         counter = 1;
         for ii = 1:specific.repetitions
            for jj = 1:nstim
               %                [dummypath fname] = fileparts(static_bi.list{1});
               %                var_labels{counter} = fname; 
               %                if (~isempty(static_bi.Rlist))
               %                   [dummypath fname] = fileparts(static_bi.Rlist{jj});
               %                   var_labels{counter} = [var_labels{counter} ' + ' fname]; 
               ...
               if isempty(static_bi.Rattens)
                  if (length(static_bi.attens) == 1)
                     var_labels{counter} = sprintf('(%d) %s, %.3f octs, %.1f dB',jj,static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens);
                  else
                     var_labels{counter} = sprintf('(%d) %s, %.3f octs, %.1f dB',jj,static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens(jj));
                  end
               elseif length(static_bi.Rattens)==1
                  if (length(static_bi.attens) == 1)
                     var_labels{counter} = sprintf('(%d) %s, %.3f octs, %.1f dB/%.1f dB',jj,static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens,static_bi.Rattens);
                  else
                     var_labels{counter} = sprintf('(%d) %s, %.3f octs, %.1f dB/%.1f dB',jj,static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens(jj),static_bi.Rattens);
                  end
               else
                  if (length(static_bi.attens) == 1)
                     var_labels{counter} = sprintf('(%d) %s, %.3f octs, %.1f dB/%.1f dB',jj,static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens,static_bi.Rattens(jj));
                  else
                     var_labels{counter} = sprintf('(%d) %s, %.3f octs, %.1f dB/%.1f dB',jj,static_bi.Used.Features_List{jj},static_bi.Used.OctShifts_List(jj),static_bi.attens(jj),static_bi.Rattens(jj));
                  end
               end
...
% If first line or using list file or multiple UpdateRates, need to load waveform from file:
if ((common.index == 1) | (length(static_bi.list) > 1) | (length(static_bi.updateRate_Hz) > 1))
   ...
   ind = mod(common.index-1,nstim)+1;
   ...
   Chan0data = refit_waveform(Chan0data,ur_Hz);   
   
   % Window waveform using linear rise/fall:
   Chan0data = window_waveform(Chan0data,ur_Hz);
   ...
   if (length(static_bi.list) > 1)
      ...
      %       stim_info.playback_sampling_rate(1) = ur_Hz;  %MH: 11Nov2004 - only stores one of many, so take it out to avoid confusion!
   else                                           
      %       static_bi.playback_sampling_rate(1)  = ur_Hz;
   end
   
end

% A right list file exists AND (either first time through OR more than one element in list)
if (((common.index == 1) | (length(static_bi.Rlist) > 1) | (length(static_bi.updateRate_Hz) > 1)) & (length(static_bi.Rlist) >0))
   ...
   % Repeat or truncate waveform to fit requested stimulus duration:
   Chan1data = refit_waveform(Chan1data,ur_Hz);   
   
   % Window waveform using linear rise/fall:
   Chan1data = window_waveform(Chan1data,ur_Hz);
   ...
   if (length(static_bi.Rlist) > 1)
      ...
      %       stim_info.playback_sampling_rate(2) = ur_Hz;
   else                                           
      %       static_bi.playback_sampling_rate(2)  = ur_Hz;
   end
...
[statCodes retVals actualUpdateRate_Hz] = d2a(1, ur_Hz, samplesMatrix);
