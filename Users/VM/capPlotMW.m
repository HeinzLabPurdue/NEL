function capThresholdSPL=capPlotMW(picNum,calibPicNum)

srWindow=[0 0.006];
drWindow=[0.0075 0.0135];
% thresh_mag=40;
capWavesYes=1; %produce a figure plotting the CAP waveforms
RMStog = 1; %toggle using RMS or pk-pk amp for threshold calcs
x=loadPic(picNum);
...
% 		capNull2(1,arb)=max(workingCap(1,srWindowInds))-min(workingCap(1,srWindowInds)); %for mean and std
		capRMS(stimNow,attenNow) = sqrt(mean(workingCap(1,drWindowInds).^2));
        RMSNull(stimNow,attenNow) = sqrt(mean(workingCap(1,srWindowInds).^2));
        capData{stimNow,attenNow}=workingCap;
	end
end
%%need to fix RMSNull to Mag based metric or compare RMS of driven waveform to RMSNull
if RMStog
    foo = RMSNull';
else
    foo = capNull';
...
meanNull=mean(foo);
stdvNull=std(foo);
thresh_mag = meanNull+2.5*stdvNull;
...
for stimNow=1:stims   %calculate threshold
	...
	if RMStog
        ABRmag(1:attens,2)=capRMS(stimNow,:)';
    else
        ABRmag(1:attens,2)=capAmp(stimNow,:)';
    end
	ABRmag(1:attens,3)=capNull(stimNow,:)';
% 	ABRmag(1:attens,4)=thresh_mag;
	ABRmag(1:attens,4)=thresh_mag(stimNow);
	ABRmag=sortrows(ABRmag,1);
	yes_thresh = 0;
	for index = 1:attens-1,
        if (ABRmag(index,2) <= thresh_mag(stimNow)) & (ABRmag(index+1,2) >= thresh_mag(stimNow)), %find points that bracket 50% hit rate
			pts = index;
			yes_thresh = 1;
		end
    end
	if yes_thresh,
		...
		slope  = (thresh_mag(stimNow) - lo_resp) / (hi_resp - lo_resp);
...
for stimNow=1:stims
	...
	plot(-atten,capAmp(stimNow,:),'.k-',-atten,capNull(stimNow,:),'.b',...
		[-120 capThreshold(stimNow) 0],[1 1 1]*thresh_mag(stimNow),':r*');
