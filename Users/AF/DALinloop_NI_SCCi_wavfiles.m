...
%    rc_d2a = check_error_codes(statCodes, retVals, 51);
  ...
  elseif (length(static_bi.updateRate_Hz) > 1) & (length(static_bi.list) == 1)  % MH:10Nov2004 EH_SCC interleaving conditions
      if (nstim > 1)
   ...
   elseif (length(static_bi.updateRate_Hz) > 1) & (length(static_bi.list) > 1)  % MH:04Dec2008 BBN_SCCXCC interleaving conditions
      if (nstim > 1)
         static_pi.var_name = 'Condition';
         static_pi.var_unit = '';
         static_pi.var_frmt = '%s';
         ...
         for ii = 1:specific.repetitions
            for jj = 1:nstim
               ...
               %                end
              % AF 7/7/2025
              [pathstr,name,ext]=fileparts(static_bi.Used.Features_List{jj});
%               [pathstr,name,ext,vers]=fileparts(static_bi.Used.Features_List{jj});
               if (length(static_bi.attens) == 1)
                   var_labels{counter} = sprintf('(%d) %s%s, %.3f octs, %.1f dB',jj,name, ...
                      ext,static_bi.Used.OctShifts_List(jj),static_bi.attens);
               else
                  var_labels{counter} = sprintf('(%d) %s%s, %.3f octs, %.1f dB',jj,name, ...
                     ext,static_bi.Used.OctShifts_List(jj),static_bi.attens(jj));
               end
               if (ii > 1)
