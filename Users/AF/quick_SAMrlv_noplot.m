function [BML,BML_ind,Rate_sps,R,level]=quick_SAMrlv_noplot(picnum,calibpic)
...
%for i=1:length(n)
	...
	for j=1:length(level)
		...
		[currentbin, binwidth, nspikes]=plotpst(sptime,nspikes,nbins,fm,spwindow);
...
% 		title(strcat('SPL=',num2str(l2)));
		...
		[R(j,1), R(j,2)]=Rtest(currentbin,nspikes,nbins,fm);
...
%     figure(101); clf; plot(level,R(:,1),'b'); grid on;
...
%     title(sprintf('SAM tone rate level function fm %0.4f, fc %0.2f, m %0.2f',fm/1000,fc,m));
