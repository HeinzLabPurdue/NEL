...
if (exist('stimulus_vals','var') == 1)
    ...
    elseif strcmp(stimulus_vals.Inloop.mdScale_type,'dBatten') %MS - 1/5/14
        mdvalsdB=-linspace(mdMin,mdMax,no_of_mdPoints);
        mdScale=10.^(mdvalsdB./20);
        if strcmp(stimulus_vals.Inloop.includeZero,'Yes') %MS - 1/5/14
            mdScale=[0 mdScale];
        end
    end    
    ...
    for i=1:length(mdScale)
        ...
        Llist{i}=stimulus_vals.Inloop.Compiled_FileName;
        audiowrite(fullfile(SAMsignals_dir,filename),tone,fs);
    end    
    stimulus_vals.Inloop.Noise_FileName{1}=fullfile(SAMsignals_dir,'BBN.wav');
    Rlist = stimulus_vals.Inloop.Noise_FileName;
    
    lowNoiseAtten=stimulus_vals.Inloop.Tone_Attenuation-stimulus_vals.Inloop.Noise_HighAtten_re_Tone;
    highNoiseAtten=stimulus_vals.Inloop.Tone_Attenuation-stimulus_vals.Inloop.Noise_LowAtten_re_Tone;
    
    Rattens_TEMP                      = [120, highNoiseAtten :-stimulus_vals.Inloop.Noise_Atten_re_Tone_StepSize:lowNoiseAtten];
    attens_TEMP                       = stimulus_vals.Inloop.Tone_Attenuation*ones(size(Rattens_TEMP+1));  % Needed to run all levels
    Nattens=length(Rattens_TEMP);
    
    Llist_TEMP=Llist;
    Rlist_TEMP=Rlist;
    
    iii=0;
    for iATTEN=1:Nattens
        for iMD=1:length(mdScale)
            iii=iii+1;
            attens(iii)=attens_TEMP(iATTEN);
            Rattens(iii)=Rattens_TEMP(iATTEN);
            Llist{iii}=Llist_TEMP{iMD};
            Rlist{iii}=Rlist_TEMP{1};
        end
    end
...
%    stimulus_vals.Inloop.Used_UpdateRate_Hz=fs;
   ...
   Inloop.params.Rattens                     = Rattens; 
   Inloop.params.attens                      = attens;
...
IO_def.Inloop.mdScale_type      =  {'{dBatten}|logarithmic|linear'}; %MS 1/5/14 - to allow user to specify m as dB atten
% IO_def.Inloop.mdScale_type      =  {'{logarithmic}|linear'};

IO_def.Inloop.mdMin             =  {30         ''    [0 100]};
IO_def.Inloop.mdMax             =  {0        ''       [0 100]};
IO_def.Inloop.no_of_mdPoints    =  {6          ''     [1 100]};
IO_def.Inloop.includeZero      =  {'{Yes}|No'}; %MS 1/5/14

IO_def.Inloop.Noise_LowAtten_re_Tone =  {10               'dB'    [-40    40]      };
IO_def.Inloop.Noise_HighAtten_re_Tone =  {20               'dB'    [-40    40]      };
IO_def.Inloop.Noise_Atten_re_Tone_StepSize =  {5              'dB'    [0    40]      };
...
IO_def.Gating.Period           = {1100    'ms'   [50 5000] 1};
