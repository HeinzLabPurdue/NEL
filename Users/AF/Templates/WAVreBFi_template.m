...
%    OctShifts=[-0.25,-0.15,-0.05,0,0.05,0.15,0.30];  % 7 CFs: R01 (122208)
   ...
   for i=1:length(Llist)
      ...
      elseif ~isempty(findstr(Llist{i},'STM'))
          FileAttenList(i)=stimulus_vals.Inloop.BBN_A_thr-stimulus_vals.Inloop.Thr_Offset;
      else
         FileAttenList(i)=NaN;
...
IO_def.Inloop.List_File             = { {['uigetfile(''' signals_dir 'Lists\AF\STM\AF_STM.m'')']} };
...
% IO_def.Gating.Duration             = {1000       'ms'    [20 4000]};

 IO_def.Gating.Period               = {2200    'ms'   [50 5000]};
% IO_def.Gating.Period               = {1400    'ms'   [50 5000]};
